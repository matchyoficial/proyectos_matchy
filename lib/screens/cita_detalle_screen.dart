...
class _CitaDetalleScreenState extends State<CitaDetalleScreen> {
        ...
        if (data['status'] == 'finished' && !_navegandoExito && mounted) {
          ...
          if (mounted) {
            _procesarExito(
                ...
                citasFaltantes: resultados['faltan'] ?? 0
            );
          }
        }
      }
  ...
  void _gestionarCancelacion() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => CancelarCitaScreen(citaId: widget.citaId, otherUserId: widget.matchyUid)));
  }
          ...
          MatchyPageLayout(
            ...
            scrollContent: SizedBox(
              ...
              child: SingleChildScrollView(
                physics: const BouncingScrollPhysics(), padding: const EdgeInsets.only(bottom: 120),
                child: Padding(
