...
class _CitasPendientesDetalleScreenState
  ...
  Future<void> _cancelarAuto() async {
    await FirebaseFirestore.instance.collection(kCitasCollection).doc(widget.citaId).delete();
    if (!mounted) return;
    _goBackToPanel();
  }
  ...
  Future<_CandidatoFS> _hydrateCandidateFromUsers(_CandidatoFS c) async {
    ...
    try {
      ...
      return _CandidatoFS(uid: c.uid, nombre: hasName ? c.nombre : nombre, edad: hasAge ? c.edad : edad, foto: hasPhoto ? c.foto : foto);
    } catch (_) { return c; }
  }
  ...
  Future<void> _hacerMatchyFlow({required _CandidatoFS c, required _CitaFS cita}) async {
    ...
    try {
      ...
      await db.runTransaction((tx) async {
        ...
        tx.set(matchyRef, {'ownerUid': user.uid, 'candidatoUid': cFull.uid, 'lastCitaAt': FieldValue.serverTimestamp(), 'citasCount': FieldValue.increment(1), 'createdAt': FieldValue.serverTimestamp()}, SetOptions(merge: true));
        tx.set(historialRef, {'citaId': cita.docId, 'fechaHora': '${cita.fecha} ${cita.hora}', 'lugarNombre': cita.nombreLugar, 'lugarDireccion': cita.direccionLugar, 'lugarFotoPortada': cita.fotoLugar, 'createdAt': FieldValue.serverTimestamp()});
        tx.set(myMatchyRef, {'nombre': cFull.nombre, 'edad': cFull.edad, 'fotoUrl': cFull.foto, 'matchId': matchyId, 'lastInteraction': FieldValue.serverTimestamp()}, SetOptions(merge: true));
        tx.set(otherMatchyRef, {'nombre': myName, 'edad': myAge, 'fotoUrl': myPhoto, 'matchId': matchyId, 'lastInteraction': FieldValue.serverTimestamp()}, SetOptions(merge: true));
      });
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error cerrando la cita: $e')));
    } finally {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          _PremiumButton(text: 'VOLVER AL PANEL', gradient: kButtonGradientBack, onTap: _goBackToPanel),
        ],
      ),
    );
  }
        ...
        children: [
          ...
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (_, snap) {
              ...
              return GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: candidatos.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 0.86), itemBuilder: (_, i) { return _CandidatoCard(candidato: candidatos[i], busy: _busy, onTapFoto: () => _openPerfil(candidatos[i].uid), onMatchy: () async => await _hacerMatchyFlow(c: candidatos[i], cita: _cita!)); });
            },
          ),
          const SizedBox(height: 30),
          _PremiumButton(text: 'VOLVER AL PANEL', gradient: kButtonGradientBack, onTap: _goBackToPanel),
          const SizedBox(height: 12),
          _PremiumButton(text: 'BORRAR CITA', gradient: kButtonGradientDelete, onTap: () async { await FirebaseFirestore.instance.collection(kCitasCollection).doc(cita.docId).delete(); if (!mounted) return; _goBackToPanel(); }),
