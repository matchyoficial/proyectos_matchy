...
// ✅ CREAR CITA (CORREGIDO ERROR PANTALLA AMARILLA)
// 🔥 FIX: Agregado scroll al modal de sedes para evitar overflow.
...
class _CreaCitaScreenState extends State<CreaCitaScreen> {
  // ... (Tus constantes de diseño siguen igual) ...
  static const double kSizeTituloPantalla = 20.0;
  static const double kSizeNombreLugar = 26.0;
  static const double kSizeDireccion = 22.0;
  ...
  static const double kSizeNotaPie = 18.0;
  ...
  // ... (Tus funciones de fecha, hora y generación de código siguen igual) ...
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Text("VAMOS A CREAR TU CITA PERFECTA", style: TextStyle(color: Colors.white, fontSize: kSizeTituloPantalla, fontWeight: FontWeight.bold, shadows: [Shadow(color: Colors.black, blurRadius: 4)]), textAlign: TextAlign.center),
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: [
                            Text(lugar.nombre.toUpperCase(), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: kSizeNombreLugar, fontWeight: FontWeight.w900, height: 1.0, shadows: [Shadow(color: Colors.black, blurRadius: 5, offset: Offset(0, 2))])),
                            const SizedBox(height: 4),
                            Text(_sedeSeleccionada != null ? _sedeSeleccionada!.direccion : lugar.direccion, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white70, fontSize: kSizeDireccion, fontWeight: FontWeight.w500, height: 1.0, shadows: [Shadow(color: Colors.black, blurRadius: 4)])),
                          ],
                        ),
                      ),
                        ...
                        _BotonPremium(
                          ...
                          onTap: _creating ? null : () async {
                            final sede = await showModalBottomSheet<SedeData>(
                              ...
                              builder: (_) => SafeArea(
                                child: Container(
                                  // Altura máxima del 70% de la pantalla para dar espacio
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      // 🔥 AQUÍ ESTÁ EL FIX: Expanded + ListView
                                      Expanded(
                      ...
                      const Padding(padding: EdgeInsets.symmetric(horizontal: 36), child: Text('Recuerda: La cita debe programarse con mínimo 12 horas de anticipación.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white54, fontSize: kSizeNotaPie, fontWeight: FontWeight.w400))),
                    ],
                  ),
                ),
              ),
...
class _BotonPremium extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Material(color: Colors.transparent, child: InkWell(borderRadius: BorderRadius.circular(_CreaCitaScreenState.kRadioBoton), onTap: onTap, child: Center(child: isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : Row(mainAxisAlignment: MainAxisAlignment.center, children: [if (icon != null) ...[Icon(icon, color: Colors.white70, size: 20), const SizedBox(width: 8)], Text(text, style: TextStyle(color: Colors.white, fontSize: fontSize, fontWeight: FontWeight.bold, letterSpacing: 0.5, fontFamily: 'Poppins'))])))),
    );
  }
}
...
class _RadioOpcion extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Row(children: [Radio<String>(value: label, groupValue: groupValue, onChanged: (v) => onChanged(v!), activeColor: Colors.white, fillColor: MaterialStateProperty.all(Colors.white), visualDensity: VisualDensity.compact), Flexible(child: Text(label, style: TextStyle(color: Colors.white, fontSize: fontSize, fontFamily: 'Poppins')))]);
  }
}